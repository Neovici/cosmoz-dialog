import{A as R,w as q,b as g,r as Z,D as J}from"./iframe-Cnwm8QaC.js";const N=e=>e??R;function F(e,t,s){return e?t(e):s?.(e)}const K=({slot:e,title:t,className:s,width:n="24",height:o="24",styles:r}={})=>g`
	<svg
		slot=${N(e)}
		class=${`clear-icon ${s??""}`}
		viewBox="0 0 24 24"
...
`,W=b(class extends p{update(){return this.state.host}}),Mt=()=>{const e=W(),t=dt(()=>{e.dispatchEvent(new Event("close")),e.onClose?.()},[]);return O(()=>{const s=e.shadowRoot;if(!s)return;const n=o=>o.target.value==="cancel"&&t();return s.addEventListener("click",n),()=>{s.removeEventListener("click",n)}},[]),{close:t}},Pt=(e,t,s)=>{const n=e.width/3,o=e.height/3,r=Math.min(window.innerWidth-2*n,Math.max(-n,t)),a=Math.min(window.innerHeight-2*o,Math.max(-o,s));return{x:r,y:a}},Lt=(e,t,s,n)=>o=>{if(!o.target.closest(t))return;const r=Pt,a=e.getBoundingClientRect(),d=o.clientX-a.x,A=o.clientY-a.y,S=(f,k)=>{const M=f-d,l=k-A,i=r(a,M,l);Object.assign(e.style,{inset:"auto",margin:"0",left:i.x+"px",top:i.y+"px",transform:"none"})},x=f=>S(f.clientX,f.clientY),v=f=>{document.removeEventListener("mousemove",x),document.removeEventListener("mouseup",v)};document.addEventListener("mousemove",x),document.addEventListener("mouseup",v)},Rt=()=>{const e=W(),{unmovable:t}=e;O(()=>{if(t)return;const s=e.shadowRoot;if(!s)return;const n=s.querySelector("dialog");if(!n)return;const o=Lt(n,".title");return s.addEventListener("mousedown",o),()=>s.removeEventListener("mousedown",o)},[t])},Dt=({title:e,content:t,closeable:s=!1,onClose:n})=>g`
	...
	`,Ot=(e,{observedAttributes:t,styles:s,...n}={})=>Et(o=>{const{close:r}=Mt();Rt();const a=mt();return O(()=>{const d=a.current;d&&!d.open&&d.isConnected&&d.showModal()},[]),g`
				<style>
					${kt}${s}
				</style>
				<dialog ${At(a)} @close=${r}>
			...
			`},{observedAttributes:["title","heading","unmovable","closeable",...t??[]],...n});customElements.define("demo-dialog",Ot(()=>g`<p>Dialog content goes here</p>`));const Nt=({heading:e,closeable:t,unmovable:s})=>g`
    <demo-dialog
        .heading=${e}
        ?closeable=${t}
        ?unmovable=${s}
